...
| Required scopes| `agents-bot--my:rw` `agents-bot--all:rw` (to update other Bot Agents within the same license) |
...
|  |  |
|--|--|
| Method URL | `https://api.livechatinc.com/v3.2/configuration/action/remove_group` |
| Required scopes | `groups--my:rw` `groups--all:rw` (to remove groups the requester is not a member of) |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.2/configuration/action/remove_group \
  -H 'Authorization: Bearer <your_access_token>' \
  -H 'Content-Type: application/json' \
  -d '{
        "id": 19
...
Otherwise, the permission depends on how the property was configured during [creation](#create-properties); parameter: `<property_name>.locations.<location>.access.<user>.write`.
...
Otherwise, the permission depends on how the property was configured during [creation](#create-properties); parameter: `<property_name>.locations.<location>.access.<user>.read`.
...
### `routing_status_set`
Informs that an Agent's or Bot Agent's status has changed.
...
| Object      | Notes                                  |
| ----------- | -------------------------------------- |
| `status`    | Possible values: `accepting chats`, `not accepting chats`, `offline` (for Bot Agents only) | 
...
| [`access_granted`](#access_granted)                                 | `chat_member_ids`, `only_my_chats`                |
| [`access_revoked`](#access_revoked)                                 | `chat_member_ids`, `only_my_chats`                |
| [`access_set`](#access_set)                                         | `chat_member_ids`, `only_my_chats`                |
...
| [`follow_up_requested`](#follow_up_requested)                       | `chat_member_ids`, `only_my_chats`                |
...
Auto access rules allow to assign groups to customer session according to their tracking url and geolocation.  Those rules are
checked on **Login** and **Update Customer Page** and resulting groups stored in customer session.  When that session starts chat with **Start Chat**
or **Activate Chat** without `chat.access` stored session groups are used as new thread's access.
...
|                |                                                                               |
| -------------- | ----------------------------------------------------------------------------- |
| Method URL     | `https://api.livechatinc.com/v3.2/configuration/action/add_auto_access_rules` |
...
| `rules`__**__   | Yes          | `object`  | Rules to check                |
| `rules.customer_url` | No      | `object`  | Rule that matches on `customer_page.url` from **Login** or `url` from **Update Customer Page** |
| `rules.customer_domain` | No   | `object`  | Rule that matches on domain of the url above |
| `rules.customer_*.values` | No | `[]match_object`_*_ | Positive match, may not be used together with `exclude_values` |
| `rules.customer_*.exclude_values` | No | `[]match_object` | Negative match |
| `rules.customer_domain` | No   | `object`  | Rule that matches on customer geolocation |
| `rules.customer_domain.values` | Yes | `[]geolocation_object`__**__ | array of objects with optional values, at least one must be set; matches when at least one subrule matches |
| `description`   | No           | `string`  | Auto chat scopes description  |

__*__ `<match_object>` structure:

| Parameter   | Required | Data type | Notes |
| ----------- | -------- | --------- | ----- |
| `value`     | Yes      | `string`  | value to match |
| `exact_match` | No     | `bool`    | if exact match, if set to `false` a `match_object.value` will be matched as substring of customer url or domain |

__**__ `geolocation_object` structure:
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.2/configuration/action/add_auto_access_rules \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <your_access_token>' \
  -d '{
...
|                |                                                                                      |
| -------------- | ------------------------------------------------------------------------------------ |
| Method URL     | `https://api.livechatinc.com/v3.2/configuration/action/get_auto_access_rules_config` |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.2/configuration/action/get_auto_access_rules_config \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <your_access_token>' \
  -d '{}'
```
...
|                |                                                                                  |
| -------------- | -------------------------------------------------------------------------------- |
| Method URL     | `https://api.livechatinc.com/v3.2/configuration/action/remove_auto_access_rules` |
| Required scopes| `access_rules:rw`                                                                |


| Parameter              | Required     | Data type | Notes                         |
| ---------------------- | ------------ | --------- | ----------------------------- |
| `auto_access_rules_id` | Yes          | `string`  | auto access rules ID          |
...
```shell
curl -X POST \
  https://api.livechatinc.com/v3.2/configuration/action/remove_auto_access_rules \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <your_access_token>' \
  -d '{
    "auto_access_rules_id": "pqi8oasdjahuakndw9nsad9na"
