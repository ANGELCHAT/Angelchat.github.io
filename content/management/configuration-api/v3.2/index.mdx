...
| Parameter      | Required     | Data type | Notes                         |
| -------------- | ------------ | --------- | ----------------------------- |
| `access`__*__  | Yes          | `object`  | Destination access            |
| `rules`__**__  | Yes          | `object`  | Rules to check for scope auto |
| `description`  | No           | `string`  | Auto chat scopes description  |

__*__ `access` object:
    * `groups`: `[1, 2]`

__**__ `rules` possible rules:
  * `customer_url.<string_filter_type>` - matches on `customer_page.url` from **Login** or `url` from **Update Customer Page**
    * `<string_filter_type>` possible values (only one is allowed for single customer_url):
      * `values` (array of `match_object`)
      * `exclude_values` (array of `match_object`)
  ...
  * `customer_domain.<string_filter_type>` - works same as rule above but works on domain part only
    * `<string_filter_type>` possible values (only one is allowed for single customer_url):
      * `values` (array of `match_object`)
      * `exclude_values` (array of `match_object`)
    * `<match_object>` structure:
      * `value` - value to match (`string`)
      * `exact_match` - if exact match, if set to `false` a `match_object.value` will be matched as substring of `customer_url`
  * `customer_geolocation.values` - array of objects with optional values, at least one must be set; matches when at least one subrule matches
    * `country`, eg. "United States
    * `country_code`, eg. "US"
    * `region`, eg. "California"
    * `city`, eg. "Mountain View"
