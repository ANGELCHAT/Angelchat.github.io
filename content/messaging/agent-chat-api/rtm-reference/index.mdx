...
### Scopes
All authorization scopes are listed in the [Scopes](#scopes-1) section. If a method requires particular authorization scopes, you'll find them included in the method description.
...
| `template_id`                    | yes      | `string`  | Defines how every Rich Message will be presented. Values: `card` or `quick_replies`.              |
| `elements`                       | no       | `array`   | Can contain up to 11 `element` objects.                                                           |
| `elements.title`                 | no       | `string`  | Displays formatted text on RMs.                                                                   |
| `elements.subtitle`              | no       | `string`  | Displays formatted text on RMs.                                                                   |
| `elements.image`                 | no       | `image`   | Displays images on RMs. Optional params: `name`, `url`, `content_type`, `size`, `width`, `height`.|
| `elements.buttons`               | no       | `array`   | Displays buttons. Can contain up to 11 `button` objects.                                          |
| `elements.buttons.type`          | no       | `array`   | Defines the behavior after a user clicks the button. Required if both `elements.buttons` and `elements.buttons.value` are present. Possible values: `webview`, `message`, `url`, `phone`. [More...](/extending-ui/extending-chat-widget/rich-messages/#getting-started)  |
| `elements.buttons.text`          | no       | `string`  | Required if `elements.buttons` is present.                                                        |
| `elements.buttons.value`         | no       | `string`  | Required if both `elements.buttons` and `elements.buttons.type` are present.                      |
| `elements.buttons.webview_height`| no       | `string`  | Possible values: `compact`, `full`, `tall`                                                        |
| `elements.buttons.postback_id`   | no       | `string`  | A description of the sent action. Required if `elements.buttons` is present. Describes the action sent via `send_rich_message_postback`. [More...](/extending-ui/extending-chat-widget/rich-messages/#getting-started) |
| `elements.buttons.user_ids`      | no       | `array`   | Required if `elements.buttons` is present. Describes users that sent the postback with `"toggled": true`.|
...
| `elements.buttons.type`          | optionally |                                                                     |
| `elements.buttons.text`          | optionally |                                                                     |
| `elements.buttons.value`         | optionally |                                                                     |
| `elements.buttons.webview_height`| optionally |                                                                     |
| `elements.buttons.postback_id`   | optionally |                                                                     |
| `elements.buttons.user_ids`      | optionally |                                                                     |
